...
extension LayoutAnchor {
    ...
    static func fullFrame(_ padding: CGFloat) -> [LayoutAnchor] {
        [.leading(padding), .trailing(padding), .top(padding), .bottom(padding)]
    }
    ...
    static func relative(
        ...
        { constant in
            .relative(
                ...
                constant: invertConstant ? -constant : constant
            )
        }
    }
    ...
    static func relativeTo(
        attribute: NSLayoutConstraint.Attribute,
        relation: NSLayoutConstraint.Relation,
        relatedTo: NSLayoutConstraint.Attribute,
        multiplier: CGFloat = 1,
        invertConstant: Bool = false
    ) -> (UIView, CGFloat) -> LayoutAnchor {
        { view, constant in
            .relativeTo(
                view: view,
                attribute: attribute,
...
extension UIView {
    ...
    func setupLayoutGuide(top: NSLayoutYAxisAnchor? = nil, bottom: NSLayoutYAxisAnchor? = nil, leading: NSLayoutXAxisAnchor? = nil, trailing: NSLayoutXAxisAnchor? = nil) -> UILayoutGuide {
        ...
        if let top { layoutGuide.topAnchor.constraint(equalTo: top).isActive = true }
        if let bottom { layoutGuide.bottomAnchor.constraint(equalTo: bottom).isActive = true }
        if let leading { layoutGuide.leadingAnchor.constraint(equalTo: leading).isActive = true }
        if let trailing { layoutGuide.trailingAnchor.constraint(equalTo: trailing).isActive = true }
    ...
    convenience init(from: UIView, to item: UIView?, anchor: LayoutAnchor) {
        ...
        case let .safeArea(attribute: attr, relation: relation, relatedTo: relatedTo, constant: constant):
            self.init(
                ...
                attribute: attr,
                relatedBy: relation,
                toItem: item?.safeAreaLayoutGuide,
                attribute: relatedTo,
                multiplier: 1,
                constant: constant
            )
        ...
        case let .keyboard(attribute: attr, relation: relation, relatedTo: relatedTo, constant: constant):
            if #available(iOS 15.0, *) {
                self.init(
                    ...
                    attribute: attr,
                    relatedBy: relation,
                    toItem: item?.keyboardLayoutGuide,
                    attribute: relatedTo,
                    multiplier: 1,
                    constant: constant
                )
